...
import { View, Text, TextInput, TouchableOpacity, StyleSheet, KeyboardAvoidingView, Platform, ImageBackground,Alert, ActivityIndicator } from 'react-native';
...
const router = useRouter();
  ...
  const handleSignUp = async () => {
    ...
    if (password.length < 6) {
      setError('Password mustbe at least 6 characters');
      return;
    }
   ...
   try {
      ...
      if (error) {
        ...
        // Profile should be created automatically, but if not, we can create it manually
       setTimeout(async () => {
          if (user) {
try {
      ...
      setError('An unexpected error occurred. Please tryagain.');
      setLoading(false);
...
<Text style={styles.logo}>Framez</Text>
           ...
           <View style={styles.form}>
             ...
             />
              ...
              {error ? <Textstyle={styles.error}>{error}</Text> : null}
